...
def select(id):
    ...
    if result is not None:
        author = Author(result['full_name'], result['id'] )
    return author
...
def update(author):
    sql = "UPDATE authors SET full_name = %s WHERE id = %s"
    values = [author.full_name, author.id]
    run_sql(sql, values)

def books(author):
    books = []

    sql = "SELECT * FROM tasks WHERE author_id = %s"
    values = [author.id]
    results = run_sql(sql, values)
